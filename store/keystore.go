...
// Ensure KeystoreStore implements AtomicStore interface
var _ AtomicStore = (*KeystoreStore)(nil)
...
func (s *KeystoreStore) GetAccounts() ([]AccountsData, []string, error) {
...
func (s *KeystoreStore) CreateWallet(name string) error {
	...
	if err != nil {
		utils.Log.Warn().Err(err).Msgf("⚠️ Keystore Store: Failed to unlock wallet after creation: %s", name)
...
func (s *KeystoreStore) GetPrivateKey(walletName string, accountName string) ([]byte, error) {
	utils.Log.Info().Msgf("🔐 Keystore Store: Getting private key for account: %s/%s", walletName, accountName)

	// Try to get account from cache first
	account, err := s.cache.FetchAccount(walletName, accountName)
	if err != nil {
		utils.Log.Error().Err(err).Msgf("❌ Keystore Store: Failed to fetch account from cache: %s/%s", walletName, accountName)
		return nil, errors.Wrap(err, "account not found in cache")
	}

	utils.Log.Debug().Msgf("🔓 Keystore Store: Extracting private key for account: %s/%s", walletName, accountName)
	key, err := getAccountPrivateKey(s.Ctx, account, s.Passphrases)
	if err != nil {
		utils.Log.Error().Err(err).Msgf("❌ Keystore Store: Failed to get private key for account: %s/%s", walletName, accountName)
		return nil, err
	}

	utils.Log.Info().Msgf("✅ Keystore Store: Successfully retrieved private key for account: %s/%s", walletName, accountName)
	return key, nil
}

func (s *KeystoreStore) SavePrivateKey(walletName string, accountName string, data interface{}) error {
	// Extract private key from interface
	privateKey, ok := data.([]byte)
	if !ok {
		return errors.New("invalid data type for Keystore store - expected []byte")
	}
...
func (s *KeystoreStore) GetCache() *WalletCache {
	return s.cache
}

// GetContext implements AtomicStore interface
func (s *KeystoreStore) GetContext() context.Context {
	return s.Ctx
}

// SetContext implements AtomicStore interface
func (s *KeystoreStore) SetContext(ctx context.Context) {
	s.Ctx = ctx
}

func newKeystoreStore(storeType string) (KeystoreStore, error) {
	store := KeystoreStore{
	...
	if storePath == "" {
		return store, errors.New("keystore store path is empty")
	}
	...
	if len(passphrases) == 0 {
		return store, errors.New("passhparases file for keystore store is empty")
	}
